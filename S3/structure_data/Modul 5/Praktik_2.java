...
public class Praktik_2 {    
    ...
    public static void selectionsort(formatBiodata biodataMahasiswa[]) {
        formatBiodata biodataSementara = new formatBiodata();
        String textTerkecil = "";
        int posisi = 0;

        for (int i = 0; i < N-2; i++) {
            textTerkecil = "zzzzzzz";
            for (int j = i+1; j < N-1; j++) {
                if(biodataMahasiswa[j].nama.compareTo(textTerkecil) < 0){
                    textTerkecil = biodataMahasiswa[j].nama;
                    posisi = j;
                }
            }
            if(biodataMahasiswa[i].nama.compareTo(biodataMahasiswa[posisi].nama) > 0) {
                biodataSementara = biodataMahasiswa[i];
                biodataMahasiswa[i] = biodataMahasiswa[posisi];
                biodataMahasiswa[posisi] = biodataSementara;
            }
        }
    ...
    public static void main(String[] args) { 
        ...
        selectionsort(biodataMahasiswa);
